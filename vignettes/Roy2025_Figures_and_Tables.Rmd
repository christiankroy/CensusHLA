...
```{r a11_by_state_prep, include=FALSE}
...
```{r a11 by us county pre, include=FALSE}
...
```{r a11_by_us_county_plot, include=FALSE}
gg_a11_by_county
```

```{r a11 by us county table}
out_data |> 
  dplyr::ungroup() |> 
  dplyr::mutate(gf = round(gf,4)) |> 
  dplyr::rename(
...
```{r a11_by_ca_county_plot}
gg_a11_in_ca
```


```{r a11_by_ca_county_table}
out_data |> 
  dplyr::ungroup() |> 
  dplyr::mutate(gf = round(gf,4)) |> 
  dplyr::rename(
...
```{r ca by hexagon prep, include=FALSE}
...
```{r ca_by_hexagon_plot}
ca_4$p1
```

```{r ca by hexagon table}
...
```{r a11 catchment prep, include=FALSE}
...
```{r a11 catchment plot, fig.height = 8, fig.width = 8}
gg_catchment
```
...
```{r b58 by county prep, include=FALSE}
...
```{r b58 by ms county prep, include=FALSE}
info_by_county <- CensusHLA::census_adjusted_nmdp_hla_frequencies_by_county |> 
  dplyr::filter(state %in% c('Mississippi')) |> 
  dplyr::filter(allele %in% c('B*58:01'))
```

```{r b58_by_ms_county_prep, include=FALSE}
out_data <- info_by_county |>
  dplyr::ungroup() |>
  dplyr::filter(allele == 'B*58:01') |>
  dplyr::group_by(region, state, census_region, county, fips, loci, allele) |>
...
```{r b58_by_ms_county_plot}
gg_b58_in_ms
```


```{r b58 by ms county table}
out_data |> 
  dplyr::ungroup() |> 
  dplyr::mutate(gf = round(gf,4)) |> 
  dplyr::rename(
...
```{r b58 ms by h4 hexagon prep, include = FALSE}
ms_4 <-
...
```{r b58_ms_by_h4_hexagon_plot, fig.height = 8, fig.width = 8}
ms_4$p1
```


```{r b58 ms by h4 hexagon table}
...
```{r b58 catchment prep, include=FALSE}
gg_catchment <- plot_delNero2022_catchment_areas(
  query_allele = 'B*58:01',
...
```{r b58 catchment plot, fig.height = 8, fig.width = 8}
gg_catchment
```

+ For a table of B\*58:01 by catchment see [Supplemental Table 4](@st4)
...
```{r supp table 1 a11 in ca county, include=FALSE}
info_by_county <- CensusHLA::census_adjusted_nmdp_hla_frequencies_by_county |> 
  dplyr::filter(state %in% c('California')) |> 
  dplyr::filter(allele %in% c('A*11:01','A*02:01','A*03:01'))
...
```{r supplemental table 1 a11 in ca county table}
out_data |> 
  dplyr::ungroup() |> 
  dplyr::mutate(gf = round(gf,4)) |> 
  dplyr::rename(
...
```{r Supp3 - b58 by mS county, include=FALSE}
info_by_county <- CensusHLA::census_adjusted_nmdp_hla_frequencies_by_county |> 
  dplyr::filter(state %in% c('Mississippi')) |> 
  dplyr::filter(allele %in% c('B*58:01'))
...
```{r supplemental table 3 b58 by ms county}
out_data |> 
  dplyr::ungroup() |> 
  dplyr::mutate(gf = round(gf,4)) |> 
  dplyr::rename(

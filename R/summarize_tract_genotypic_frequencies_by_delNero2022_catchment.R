...
summarize_tract_genotypic_frequencies_by_delNero2022_catchment <- function(query_allele) {
  ...
  futile.logger::flog.info(
    "Calculating nmdp frequencies for each census tract for all states. This will take some time"
  )
  ...
  delNero2022_sf_path <-
    paste0(
      ...
      "/extdata/delNero2022/NCI_Catchment_Areas_fall2024.shp"
    )
  delNero2022_sf <- sf::st_read(delNero2022_sf_path)
  # Transform the sf object to the target CRS
  # We need to do this because NCI/delNero2022 used a different coordinate unit
  delNero2022_sf <- sf::st_transform(delNero2022_sf, target_crs)
  # Flog that we're going to pull in state geographic area including tract data with centroids
  futile.logger::flog.info(
    "Pulling in state geographic area including tract data with centroids. This will take some time"
  )
  ...
  sf_tract_centroids_for_all_states_with_catchment <- sf::st_join(#sparse = TRUE,
    x = delNero2022_sf, y = sf_tract_centroids_for_all_states)
  ...
  return(
    list(
      sf_tract_centroids_for_all_states_with_catchment_with_us_population_race_code_percentages_by_tract_summed = sf_tract_centroids_for_all_states_with_catchment_with_us_population_race_code_percentages_by_tract_summed, # Need
      nmdp_freq = nmdp_freq#, # Need
